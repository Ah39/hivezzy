...
public class FetchOperator implements Serializable {
  ...
  private void getNextPath() throws Exception {
    ...
    while (iterPath.hasNext()) {
      ...
      /*zzy edit
       原版  是修改RecordReader  FileInputFormat.setInputPathFilter(job, FetchFilenameFilter.class);*/
      ...
      if (fs.exists(nxt)) {
        ...
        for (FileStatus fStat : fStats) {
        	LOG.error("PATH:"+fStat.getPath().getName());
        	LOG.error(this.getClass().getName()+"location:"+location);
          if (fStat.getLen() > 0 && Pattern.compile(location).matcher(fStat.getPath().getName()).matches()) {
        	  /*zzy edit Pattern.compile(location).matcher(onepath.getName()).matches()*/
            currPath = nxt;
            if (iterPartDesc != null) {
              currPart = prt;
            }
