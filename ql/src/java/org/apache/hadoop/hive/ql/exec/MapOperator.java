...
public class MapOperator extends Operator<MapWork> implements Serializable, Cloneable {
 ...
 private static Log LOG = LogFactory.getLog("mapoperator");
  ...
  public void setChildren(Configuration hconf) throws HiveException {
    ...
    try {
      for (Map.Entry<String, ArrayList<String>> entry : conf.getPathToAliases().entrySet()) {
       ...
       String location = tblProps.getProperty(hive_metastoreConstants.META_TABLE_LOCATION);
       /*@zzy
        * @ get location
        */
       LOG.error("ZZY:TEST:_"+"LOCATION:"+location);
       LOG.error("FILE_NAME:"+onefile);
       LOG.error("PATH_NAME:"+onepath.getName());
        ...
        //  if (!Pattern.compile(fileNameFilter).matcher(mapFilePath.getName()).matches()) {
           //   continue;
          if(!Pattern.compile(location).matcher(onepath.getName()).matches())
          {
        	  LOG.error("match_fail");
        	  continue;
          }
